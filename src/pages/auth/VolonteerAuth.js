...
import {VOLONTEER_ROUTE} from "../../utils/Consts";
...
const VolonteerAuth = () => {
    ...
    const click = async () => {
        try {
            ...
            user.setIsAuth(true)
            navigate(VOLONTEER_ROUTE, {replace: true})
        } catch (e) {
            user.setIsAuth(true)
            navigate(VOLONTEER_ROUTE, {replace: true})
    ...
    return (
        <div>
            <h2 className="m-auto text-center">Введите данные для регистрации в Telegram-боте</h2>
            <Form className="d-flex flex-column">
                <Form.Control className="mt-3" placeholder="Введите регион"
                              onChange={e => setRegion(e.target.value)}/>
                <Form.Control className="mt-3" placeholder="Введите город"
                ...
                <Form.Control className="mt-3" placeholder="Введите опыт"
